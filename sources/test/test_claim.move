#[test_only]
module kriya_fountain::test_claim {
    ...
    fun test_wrong_fountain_id() {
        ...
        let scenario = &mut scenario_val;
        let stakers = ftu::stake_randomly<TEST_COIN_A, TEST_COIN_B, SUI>(scenario, 5);
        ...
        ts::next_tx(scenario, staker);
        {
            let clock = ts::take_shared<Clock>(scenario);
            let current_time = clock::timestamp_ms(&clock);
            let init_token = kriya::spot_dex::new_for_testing(pool_id(), 0, ts::ctx(scenario));
