#[test_only]
module kriya_fountain::test_force_unstake {
    use std::vector;
    use sui::sui::SUI;
    ...
    use kriya_fountain::fountain_core::{Self as fc, Fountain, StakeProof, AdminCap};
    ...
    fun test_force_unstake() {
        ...
        ts::next_tx(scenario, ftu::dev());
        {
            ...
            let init_token = spot_dex::new_for_testing<TEST_COIN_A, TEST_COIN_B>(pool_id(), 0, ts::ctx(scenario));
            fc::new_penalty_vault(&admin_cap, &mut fountain, init_token, max_penalty_rate);
        ...
        {
            let fountain = ts::take_shared<Fountain<TEST_COIN_A, TEST_COIN_B, SUI>>(scenario);
            let penalty_lp = ts::take_from_sender<KriyaLPToken<TEST_COIN_A, TEST_COIN_B>>(scenario);
            assert!(fc::get_penalty_vault_balance(&fountain) == 0, 0);
            assert!(spot_dex::lp_token_value(&penalty_lp) == total_penalty_amount, 0);
            ts::return_shared(fountain);
